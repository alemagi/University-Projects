...
We are working with space-temporal data from the U.S. Environmental Protection Agency (EPS). The dataset displays hourly air quality data measured in 10 different stations in California from June to September 2020. In particular, we have decided to focus and analyze the data of station 95 (station_id 95). 
...
```{r Hourly Graph, echo=FALSE, eval = TRUE, out.width='50%', fig.show='hold', fig.cap='Time Series by Hour at Station 95'}
...
```{r Half Day Graph, echo=FALSE, eval = TRUE, out.width='50%', fig.show='hold', fig.cap='Time Series Mean Over Half-Day at Station 95'}
...
```{r Collapsed Graphs, echo=FALSE, eval = TRUE, out.width='50%', fig.show='hold', fig.cap='Time Series Mean Over Day at Station 95'}
...
We proceed with the specification of the following model with 5 latent states: 
...
Y_t = \mu_5 + \epsilon_t, \quad \epsilon_t \overset{iid}{\sim} N(0, \sigma_5^2) 
&\text{if the state $S_t=5$}.
\end{cases}
\end{equation*}
...
set.seed(2022) 
model <- depmix(id_95_coll$pm25 ~ 1, data=id_95_coll, nstates=5)
fmodel <- fit(model)
summary(fmodel)
...
in_prob <- data.frame(0,0,0,0,0)
in_prob[nrow(in_prob),] <- MLEse[1:5,1]
in_prob[nrow(in_prob)+1,] <- MLEse[1:5,3]
...
kable(in_prob, 
      ...
      col.names = c("Prob. 1", "Prob. 2", "Prob. 3", "Prob. 4", "Prob. 5"),
      align = 'c')
...
trans_mat <- data.frame(0,0,0,0,0)
trans_mat[nrow(trans_mat),] <- MLEse[6:10,1]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[6:10,3]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[11:15,1]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[11:15,3]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[16:20,1]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[16:20,3]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[21:25,1]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[21:25,3]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[26:30,1]
trans_mat[nrow(trans_mat) + 1,] <- MLEse[26:30,3]

trans_mat <- data.frame(trans_mat, row.names = c("From State 1", "$se_{1}$","From State 2", "$se_{2}$","From State 3", "$se_{3}$","From State 4", "$se_{4}$","From State 5", "$se_{5}$"))
...
trans_mat[10,] <- paste0("(", format(unlist(trans_mat[10,])),")")

kable(trans_mat, 
      digits = 4, 
      caption = "Transition Matrix",
      col.names = c("To 1", "To 2", "To 3", "To 4", "To 5"),
      align = 'c')
...
em_dist <- data.frame(0,0,0,0,0)
em_dist[nrow(em_dist),] <- MLEse[c(31,33,35,37,39),1]
em_dist[nrow(em_dist) + 1,] <- MLEse[c(31,33,35,37,39),3]
em_dist[nrow(em_dist) + 1,] <- MLEse[c(32,34,36,38,40),1]
em_dist[nrow(em_dist) + 1,] <- MLEse[c(32,34,36,38,40),3]
...
kable(em_dist, 
      ...
      col.names = c("State 1", "State 2", "State 3", "State 4", "State 5"),
      align = 'c')
...
estMean5=fmodel@response[[5]][[1]]@parameters$coefficients
...
estMeans[estStates[,1]==5]=estMean5
...
model <- depmix(id_95_half$pm25 ~ 1, data=id_95_half, nstates=5)
fmodel <- fit(model)
summary(fmodel)
...
graph_states <- id_95_states_long %>%
  ...
  aes( x = datetime,
       y = value,
       color = type) + 
  labs(x= "Time", title = "B: PM2.5 Level and Optimal State Process") +
  ylab("") +
  geom_line(show.legend = FALSE) +
  facet_wrap(~ type, ncol = 1, scale="free_y") + 
  theme_minimal()
...
graph_means <- id_95_states_means_long %>%
  ...
  aes( x = datetime,
       y = value,
       color = type) + 
  labs(x= "Time", title = "A: PM2.5 Level with Period Specific Means" ) +
  ylab("") +
  geom_line(show.legend = FALSE) + 
  theme_minimal()
...
set.seed(2022) #With this seed the higher state is 3, the lower is 1 and the middle is 2
model <- depmix(id_95$pm25 ~ 1, data=id_95, nstates=5)
fmodel <- fit(model)
summary(fmodel)
